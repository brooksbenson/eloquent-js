...
function compareRobots(robots) {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    robots.forEach((r, i) => {
      results[r.name] = testRobot(r.routine, r.memory, 1, state);
    });
  }
...
/*
  At any point, a robot either needs to be picking
  up or dropping off a parcel. The goal oriented
  robot always makes its decision based on
  the first parcel in the list without evaluating
  if picking up or dropping off another parcel in
  the list would be a good idea.

  The customRobot differs because when it picks
  a new route, it evaluates a route for every
  parcel on the list, then picks the shortest 
...
console.log(compareRobots(robots));
